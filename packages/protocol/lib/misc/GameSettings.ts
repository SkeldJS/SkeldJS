...
export class GameSettings {
    ...
    Deserialize(reader: HazelReader) {
        const versionOrLength = reader.upacked();
        const hasLength = versionOrLength > 40;

        const settingsReader = hasLength ? reader.bytes(versionOrLength) : reader;
        const version = hasLength ? settingsReader.uint8() : versionOrLength;
        ...
        if (version >= 7) {
            ...
            if (this.gameMode === GameMode.HideNSeek) {
                ...
                this.roleSettings.Deserialize(mreader, versionOrLength);
            }
        } else {
            ...
            if (version >= 2) {
                ...
                if (version >= 3) {
                    ...
                    if (version >= 4) {
                        ...
                        if (version >= 5) {
                            this.roleSettings.Deserialize(settingsReader, version);
                        }
                    }
                }
            }
