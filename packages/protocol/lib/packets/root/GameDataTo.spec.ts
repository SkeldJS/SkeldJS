...
import { GameDataToMessage } from "./GameDataToMessage";
...
describe("GameDataToMessage", () => {
    describe("GameDataToMessage#Deserialize", () => {
        it("Should deserialize a alter game root message.", () => {
            ...
            const reader = HazelReader.from("48daca8cbfaf0221000403feffffff0f00025e1000010100076861696c657920076d020000005f010001001e000404e9ad02010360020001000061000001620a00010100c27ab286ff7fff7f0c000402feffffff0f000163000001", "hex");
            const packet = GameDataToMessage.Deserialize(reader, MessageDirection.Clientbound, decoder);

            assert.strictEqual(packet.tag, RootMessageTag.GameDataTo);
            ...
            assert.deepStrictEqual(packet._children.map(child => child.tag), [
    ...
    describe("GameDataToMessage#Serialize", () => {
        it("Should serialize a alter game root message.", () => {
            ...
            const packet = new GameDataToMessage(
                "GITWMF",
                38847,
                [
            ...
            assert.strictEqual(writer.toString("hex"), "48daca8cbfaf02000004000004000004");
        });
    });
});
