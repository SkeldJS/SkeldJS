...
import { Packet, ClientboundPacket, ServerboundPacket } from "./packets";
                    ...
                    case PayloadTag.JoinGame:
                        if (payload.bound === "client") {
                            if (payload.error) {
                                ...
                                if (
                                    payload.reason === DisconnectReason.Custom
                                ) {
                                ...
                                case MessageTag.RPC:
                                    ...
                                    switch (message.rpcid) {
                                        ...
                                        case RpcTag.SetInfected:
                                            writer.upacked(
                                                message.impostors.length
                                            );
                                            for (
                                                let i = 0;
                                                i < message.impostors.length;
                                                i++
                                            ) {
                                                writer.uint8(
                                                    message.impostors[i]
