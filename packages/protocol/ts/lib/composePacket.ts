...
import { HazelBuffer, writeVector2 } from "@skeldjs/util";
...
export function composeGameData(message: GameDataMessage): HazelBuffer {
        ...
        case MessageTag.RPC:
            ...
            switch (message.rpcid) {
                ...
                case RpcTag.ClimbLadder:
                    writer.uint8(message.ladderId);
                    writer.uint8(message.seqId);
                    break;
...
export function composePayload(payload: PayloadMessage): HazelBuffer {
    const writer = HazelBuffer.alloc(512);
    ...
    switch (payload.tag) {
        ...
        case PayloadTag.GameDataTo:
            ...
            for (let i = 0; i < payload.messages.length; i++) {
                ...
                writer.buf(composeGameData(message));
            }
            break;
    ...
    switch (packet.op) {
        ...
        case Opcode.Unreliable:
            for (let i = 0; i < packet.payloads.length; i++) {
                ...
                writer.buf(composePayload(payload));
            }
            break;
        case Opcode.Hello:
